...
const os = require('os'), flags = require('./flags'), hash = require('./hash');
const NTLMSIGNATURE = "NTLMSSP\0";
function createType1Message(workstation, target) {
    let dataPos = 32, pos = 0, buf = new Buffer.alloc(1024);
...
function decodeType2Message(str) {
    ...
    let buf = new Buffer.from(str, 'base64'), obj = {};
    //check signature
...
function createType3Message(type2Message, username, password, workstation, target) {
    let dataPos = 52, buf = new Buffer.alloc(1024);
    ...
    if (type2Message.version === 2) {
        dataPos = 64;
        let ntlmHash = hash.createNTLMHash(password), nonce = hash.createPseudoRandomValue(16), lmv2 = hash.createLMv2Response(type2Message, username, ntlmHash, nonce, target), ntlmv2 = hash.createNTLMv2Response(type2Message, username, ntlmHash, nonce, target);
        ...
        let lmHash = hash.createLMHash(password), ntlmHash = hash.createNTLMHash(password), lm = hash.createLMResponse(type2Message.challenge, lmHash), ntlm = hash.createNTLMResponse(type2Message.challenge, ntlmHash);
        //lm security buffer
