...
function createLMResponse(challenge, lmhash) {
    let buf = new Buffer.alloc(24), pwBuffer = new Buffer.alloc(21).fill(0);
...
function createLMHash(password) {
    let buf = new Buffer.alloc(16), pwBuffer = new Buffer.alloc(14), magicKey = new Buffer.from('KGS!@#$%', 'ascii');
...
function createNTLMResponse(challenge, ntlmhash) {
    let buf = new Buffer.alloc(24), ntlmBuffer = new Buffer.alloc(21).fill(0);
...
function createLMv2Response(type2message, username, ntlmhash, nonce, targetName) {
    let buf = new Buffer.alloc(24), ntlm2hash = createNTLMv2Hash(ntlmhash, username, targetName), hmac = crypto.createHmac('md5', ntlm2hash);
    //server challenge
...
function createNTLMv2Response(type2message, username, ntlmhash, nonce, targetName) {
    let buf = new Buffer.alloc(48 + type2message.targetInfo.buffer.length), ntlm2hash = createNTLMv2Hash(ntlmhash, username, targetName), hmac = crypto.createHmac('md5', ntlm2hash);
